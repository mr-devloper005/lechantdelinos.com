...
const fieldClass = 'rounded-xl border border-[var(--editable-border)] bg-white px-4 py-3 text-sm font-bold text-[#092f2c] outline-none transition placeholder:text-[#6f8580] focus:border-[#0b7669] focus:ring-4 focus:ring-[#0b7669]/10'
...
export default function CreatePage() {
  ...
  if (!session) {
    return (
      <EditableSiteShell>
        <main className="min-h-screen bg-[var(--editable-page-bg,#f4fbf7)] px-4 py-14 text-[var(--editable-page-text,#092f2c)] sm:px-6 lg:px-8">
          <section className="mx-auto grid max-w-5xl gap-8 rounded-2xl border border-[var(--editable-border)] bg-white p-7 shadow-[0_30px_90px_rgba(15,23,42,0.08)] md:grid-cols-[0.9fr_1.1fr] md:p-10">
            <div className="flex h-full min-h-72 items-center justify-center rounded-2xl bg-[#0b1d2e] text-white">
            ...
            <div className="self-center">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/login" className="inline-flex items-center gap-2 rounded-full bg-[#0b1d2e] px-6 py-3 text-sm font-black text-white">Login <ArrowRight className="h-4 w-4" /></Link>
    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-[var(--editable-page-bg,#f4fbf7)] text-[var(--editable-page-text,#092f2c)]">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="grid gap-8 rounded-2xl border border-[var(--editable-border)] bg-white p-6 shadow-[0_30px_90px_rgba(15,23,42,0.08)] lg:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <aside>
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-2">
                {enabledTasks.map((item) => {
                  ...
                  return (
                    <button key={item.key} type="button" onClick={() => setTask(item.key)} className={`rounded-xl border p-4 text-left transition ${active ? 'border-current bg-[#0b1d2e] text-white' : 'border-[var(--editable-border)] bg-[#f7faf8] hover:-translate-y-0.5'}`}>
            ...
            <form onSubmit={submit} className="rounded-2xl border border-[var(--editable-border)] bg-[#f7faf8] p-5 sm:p-7">
              ...
              <div className="mt-6 grid gap-4">
                ...
                <textarea className={`${fieldClass} min-h-48`} value={body} onChange={(event) => setBody(event.target.value)} placeholder="Services, location, hours, contact notes, and business description" required />
              </div>
              ...
              <button type="submit" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#0b1d2e] px-6 text-sm font-black uppercase tracking-[0.12em] text-white transition hover:-translate-y-0.5">
