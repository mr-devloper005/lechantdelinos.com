...
import { Building2, LogIn, Menu, PlusCircle, Search, UserPlus, X } from 'lucide-react'
...
export function EditableNavbar() {
  ...
  const navVars = { '--editable-nav-bg': preset.colors.background, '--editable-nav-text': preset.colors.foreground, '--editable-nav-active': '#f36c4f', '--editable-nav-active-text': '#ffffff', '--editable-cta-bg': '#0b1d2e', '--editable-cta-text': '#ffffff', '--editable-search-bg': preset.colors.surface, '--editable-border': `${preset.colors.muted}33`, '--editable-container': '1180px' } as CSSProperties
      ...
      <nav className="mx-auto flex min-h-[84px] w-full max-w-[var(--editable-container)] items-center gap-4 px-4 sm:px-6 lg:px-8">
        <Link href="/" className="group flex shrink-0 items-center gap-3">
          ...
          <span className="hidden min-w-0 sm:block">
            <span className="block max-w-[180px] truncate text-lg font-black tracking-tight">{brandName}</span>
        ...
        <div className="hidden items-center gap-2 lg:flex">
          {navItems.slice(0, 4).map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-black transition ${active ? 'bg-[var(--editable-nav-active)] text-[var(--editable-nav-active-text)] shadow-sm' : 'bg-white/55 hover:bg-white'}`}>
                {item.href === '/listing' ? <Building2 className="h-4 w-4" /> : null}
                {item.label}
              </Link>
            )
          })}
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          {session ? (
            <>
              ...
              <span className="hidden max-w-[150px] truncate rounded-full bg-white/70 px-4 py-2 text-sm font-black sm:inline-flex">{session.name}</span>
              <button type="button" onClick={logout} className="hidden items-center gap-2 rounded-full px-3 py-2 text-sm font-black hover:bg-white/70 sm:inline-flex">Logout</button>
            </>
          ) : (
            <>
              <Link href="/login" className="hidden items-center gap-2 rounded-full bg-white/55 px-3 py-2 text-sm font-black hover:bg-white sm:inline-flex"><LogIn className="h-4 w-4" /> Login</Link>
      ...
      {open ? (
        <div className="border-t border-[var(--editable-border)] bg-[var(--editable-nav-bg)] px-4 py-4 lg:hidden">
          ...
          <div className="grid gap-2">
            {[{ label: 'Home', href: '/' }, ...navItems, ...(session ? [{ label: session.name, href: '/create' }] : [{ label: 'Login', href: '/login' }, { label: 'Sign up', href: '/signup' }])].map((item) => (
